...
<jsp:include page="/mainpage/top.jsp"/>
	<form name="joinform" action="updateProcess.net" method="post"
		...
		<hr>
		<b>아이디</b> 
		<input type="text" name="mem_id" value="${memberinfo.mem_id}" readOnly> 
		
	    <b>비밀번호</b>
	    <input type="password" name="mem_password" value="${memberinfo.mem_password}" readOnly>
			
		<b>이름</b>
		<input type="text" name="mem_name" value="${memberinfo.mem_name}" placeholder="Enter name"
			maxLength="5" required> 
			
		<b>닉네임</b>
        <input type="text" name="mem_nickname" value="${memberinfo.mem_nickname}" placeholder="Enter name" maxLength="20" required>		
		...
		<b>연락</b>
        <input type="text" name="mem_tel" value="${memberinfo.mem_tel}" placeholder="Enter tel" maxLength="17" required>
			
		<b>이메일 주소 </b>
		<input type="text" name="mem_email"	value="${memberinfo.mem_email}" 
					placeholder="Enter email" maxLength="30" required>
		<span id="email_message"></span>
		
		<b>나이</b>
		<input type="text" name="mem_birth"	value="${memberinfo.mem_birth}" maxLength="2" 
				placeholder="Enter age" required> 
				
				
		<b>주소</b>
<input type="text" name="mem_address" value="${memberinfo.mem_address}" placeholder="Enter address" maxLength="200" required>
<b>타입</b>
<div>
...
<script>
	//성별 체크해주는 부분
	$("input[value='${memberinfo.mem_gender}']").prop('checked',true);
	...
	$("input:eq(6)").on('keyup',
		...
		$('form').submit(function() {
			if (!$.isNumeric($("input[name='mem_birth']").val())) {
				alert("나이는 숫자를 입력하세요");
				$("input[name='mem_birth']").val('').focus();
				return false;
			}			
