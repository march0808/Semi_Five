...
public class htBoardListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		htBoardDAO htboarddao = new htBoardDAO();
		List<htBoardBean> htboardlist = new ArrayList<htBoardBean>();
		...
		// 총 리스트 수를 받아옵니다.
		int listcount = htboarddao.getListCount();
		
		// 리스트를 받아옵니다.
		htboardlist = htboarddao.getHtboardList(page, limit);
		 ...
		 * 최대페이지(maxpage)가 25라면 [21]~[25]까지만 표시되도록 합니다.
		 */
		if (endpage > maxpage)
			endpage = maxpage;
		...
		if (state == null) {
		    ...
		    request.setAttribute("htboardlist", htboardlist);
		    request.setAttribute("limit", limit);
		    ActionForward forward = new ActionForward();
		    forward.setRedirect(false);
		    
		    // 글 목록 페이지로 이동하기 위해 경로를 설정합니다.
		    forward.setPath("htboard/htboardList.jsp");
		    return forward;
		   ...
		   //List => JsonElement
		   JsonElement je = new Gson().toJsonTree(htboardlist);
		   System.out.println("htboardlist="+je.toString());
		   object.add("htboardlist", je);
