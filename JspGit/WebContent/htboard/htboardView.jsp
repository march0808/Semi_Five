...
<body>
	...
	<div class="container">
		<table class="table">
			<tr>
				<th colspan="2">건강 QnA 게시판- view 페이지</th>
			</tr>
			<tr>
				<td><div>글쓴이</div></td>
				<td><div>${boarddata.ht_nickname}</div></td>
			</tr>
			<tr>
				<td><div>제목</div></td>
				<td><c:out value="${boarddata.ht_board_title}" /></td>
			</tr>
			<tr>
				<td><div>내용</div></td>
				<td style="padding-right: 0px"><textarea class="form-control"
						rows="5" readOnly>${boarddata.ht_board_content}</textarea></td>
			</tr>

			<c:if test="${boarddata.ht_re_lev==0}">
				<%--원문글인 경우에만 첨부파일을 추가할 수 있습니다. --%>
				<tr>
					<td><div>첨부파일</div></td>
					<c:if test="${!empty boarddata.ht_board_file}">
						<%-- 파일첨부한 경우 --%>
						<td><img src="image/down.png" width="10px"> <a
							href="htBoardFileDown.htbo?filename=${boarddata.ht_board_file}">
								${boarddata.ht_board_file}</a></td>
					</c:if>
					<c:if test="${empty boarddata.ht_board_file}">
						<%-- 파일첨부하지 않은 경우 --%>
						<td></td>
			...
			<tr>
				<td colspan="2" class="center"><a
					href="htBoardReplyView.htbo?num=${boarddata.ht_board_idx}">
						<button class="btn btn-primary">답변</button>
				</a> <c:if test="${boarddata.ht_nickname == id || id == 'admin' }">
						<a href="htBoardModifyView.htbo?num=${boarddata.ht_board_idx}">
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form name="deleteForm" action="htBoardDeleteAction.htbo"
							method="post">
               ...
               --%>
							...
							<div class="form-group">
								<label for="pwd">비밀번호</label> <input type="password"
									...
									name="ht_board_pass" id="board_pass">
							</div>
